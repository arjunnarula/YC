...
let BeboltAlertActionEnabledDidChangeNotification = "BeboltAlertActionEnabledDidChangeNotification"

public enum BeboltAlertActionStyle : Int {
    case `default`
    case cancel
    case destructive
}

public enum BeboltAlertControllerStyle : Int {
    case actionSheet
    case alert
}

// MARK: BeboltAlertAction Class
open class BeboltAlertAction : NSObject, NSCopying {
    open var title: String
    open var style: BeboltAlertActionStyle
    var handler: ((BeboltAlertAction?) -> Void)!
    open var enabled: Bool {
        didSet {
            if (oldValue != enabled) {
                NotificationCenter.default.post(name: Notification.Name(rawValue: BeboltAlertActionEnabledDidChangeNotification), object: nil)
            }
        }
    }
    
    required public init(title: String, style: BeboltAlertActionStyle, handler: ((BeboltAlertAction?) -> Void)!) {
        self.title = title
        self.style = style
...
// MARK: BeboltAlertAnimation Class
open class BeboltAlertAnimation : NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func presentAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as! BeboltAlertController
    ...
    func dismissAnimateTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        ...
        let alertController = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as! BeboltAlertController
...
// MARK: BeboltAlertController Class
@objc(BeboltAlertController)
open class BeboltAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    
    // Message
    open var message: String?
    
    // AlertController Style
    fileprivate(set) var preferredStyle: BeboltAlertControllerStyle?
    ...
    open var titleFont =  UIFont(name: "Poppins-Bold", size: 18) ?? UIFont.boldSystemFont(ofSize: 18)
    open var titleTextColor = UIColor.black
    
    // MessageView
    open var messageView = UILabel()
    open var messageFont = UIFont(name: "Poppins-Regular", size: 15) ?? UIFont.systemFont(ofSize: 15)
    ...
    // Buttons
    var buttons = [Button]()
    open var buttonFont: [BeboltAlertActionStyle : UIFont] = [
        .default : UIFont(name: "Poppins-Regular", size: 16) ?? UIFont.systemFont(ofSize: 16),
        .cancel  : UIFont(name: "Poppins-Regular", size: 16) ?? UIFont.systemFont(ofSize: 16),
        .destructive  : UIFont(name: "Poppins-Regular", size: 16) ?? UIFont.systemFont(ofSize: 16)
    ]
    open var buttonTextColor: [BeboltAlertActionStyle : UIColor] = [
        .default : UIColor.white,
        .cancel  : UIColor.white,
        .destructive  : UIColor.white
    ]
    open var buttonBgColor: [BeboltAlertActionStyle : UIColor] = [
        .default : UIColor(red:52/255, green:152/255, blue:219/255, alpha:1),
        .cancel  : UIColor(red:85/255, green:85/255, blue:85/255, alpha:1),
        .destructive  : UIColor(red:231/255, green:76/255, blue:60/255, alpha:1)
    ]
    open var buttonBgColorHighlighted: [BeboltAlertActionStyle : UIColor] = [
    ...
    fileprivate var buttonCornerRadius: CGFloat = 22.0
    ...
    // Initializer
    public convenience init(title: String?, message: String?, preferredStyle: BeboltAlertControllerStyle) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(BeboltAlertController.handleAlertActionEnabledDidChangeNotification(_:)),
                                               name: NSNotification.Name(rawValue: BeboltAlertActionEnabledDidChangeNotification), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(BeboltAlertController.handleKeyboardWillShowNotification(_:)),
                                               name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(BeboltAlertController.handleKeyboardWillHideNotification(_:)),
                                               name: UIResponder.keyboardWillHideNotification, object: nil)
        
        //add observor for notification here
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if (!isAlert() && cancelButtonTag != 0) {
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(BeboltAlertController.handleContainerViewTapGesture(_:)))
    ...
    open func layoutView(_ presenting: UIViewController?) {
        ...
        if ((UIDevice.current.systemVersion as NSString).floatValue < 8.0) {
            if UIInterfaceOrientation.landscapeLeft == currentOrientation() {
        ...
        alertView.layer.shadowRadius = 7
        alertView.layer.shadowOpacity = 0.5
        alertView.layer.shadowOffset = CGSize(width: 1, height: 5)
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! BeboltAlertAction
                ...
                if action.style == .cancel {
                    ...
                    button.applyBackgroundGradient = true
                }
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! BeboltAlertAction
                if (action.style != BeboltAlertActionStyle.cancel) {
                    button.titleLabel?.font = buttonFont[action.style]
            ...
            if (cancelButtonTag != 0) {
                ...
                let action = actions[cancelButtonTag - 1] as! BeboltAlertAction
                button.titleLabel?.font = buttonFont[action.style]
                button.setTitleColor(buttonTextColor[action.style], for: UIControl.State())
                button.backgroundColor = buttonBgColor[action.style]
                button.frame = CGRect(x: 0, y: buttonAreaPositionY, width: innerContentWidth, height: buttonHeight)
    ...
    @objc func buttonTapped(_ sender: UIButton) {
        ...
        let action = actions[sender.tag - 1] as! BeboltAlertAction
    ...
    open func addAction(_ action: BeboltAlertAction) {
        // Error
        if (action.style == BeboltAlertActionStyle.cancel) {
            for ac in actions as! [BeboltAlertAction] {
                if (ac.style == BeboltAlertActionStyle.cancel) {
                    let error: NSError? = nil
                    NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format:"BeboltAlertController can only have one action with a style of BeboltAlertActionStyleCancel", arguments:getVaList([error ?? "nil"]))
                    return
                }
            }
        }
        ...
        button.addTarget(self, action: #selector(BeboltAlertController.buttonTapped(_:)), for: .touchUpInside)
    ...
    open func addTextFieldWithConfigurationHandler(_ configurationHandler: ((UITextField?) -> Void)!) {
        ...
        // You can add a text field only if the preferredStyle property is set to BeboltAlertControllerStyle.Alert.
        if (!isAlert()) {
            let error: NSError? = nil
            NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format: "Text fields can only be added to an alert controller of style BeboltAlertControllerStyleAlert", arguments:getVaList([error ?? "nil"]))
            return
        }
        if (textFields == nil) {
            textFields = []
    ...
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        ...
        return BeboltAlertAnimation(isPresenting: true)
    }
    
    open func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return BeboltAlertAnimation(isPresenting: false)
    }
}
